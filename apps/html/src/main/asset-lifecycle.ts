...
export interface OwnedAssetManifest {
  ...
  /** HTML file name -> app-owned files referenced by its last successful save. */
...
async function existingAssetsDir(documentPath: string): Promise<string | null> {
  ...
  try {
    ...
    if (!info.isDirectory() || info.isSymbolicLink()) {
      throw new Error('html assets path must be a real directory')
    }
    return assetsDir
  } catch (error) {
...
async function createOwnedAsset(
  ...
  throw new Error('html asset name space exhausted')
}
...
async function siblingHtmlReferences(
  documentPath: string,
...
export async function prepareAssetsForSaveAs(
  ...
  for (const source of extractHtmlImageSources(text)) {
  ...
  try {
    await withManifestLock(targetDocumentPath, async () => {
      ...
      for (const source of allImageSources) {
        ...
        if (!copied) throw new Error('html asset name space exhausted')
        created.push(copied)
      ...
      const rewrittenDestinations = new Set(
        extractHtmlImageSources(rewriteHtmlImageSources(text, rewrites)),
      )
  ...
  return {
    targetDocumentPath,
    text: rewriteHtmlImageSources(text, rewrites),
...
async function resolvePendingOwnedAssets(
  ...
  try {
    return await withManifestLock(documentPath, async () => {
      ...
      for (const name of await siblingHtmlReferences(documentPath, ownedNames, true)) {
        referenced.add(name)
      }
...
export async function reconcileOwnedAssets(
  ...
  try {
    return await withManifestLock(documentPath, async () => {
      ...
      for (const name of await siblingHtmlReferences(documentPath, ownedNames)) {
        referenced.add(name)
      }
