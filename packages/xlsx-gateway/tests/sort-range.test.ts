...
describe('computeSortedRowOrder non-finite keys', () => {
  it('sorts finite numbers deterministically around non-finite keys', () => {
    ...
    const first = computeSortedRowOrder(rows.map((r) => [...r]), 0, true)
    ...
    for (let k = 0; k < 10; k++) {
      expect(computeSortedRowOrder(rows.map((r) => [...r]), 0, true)).toEqual(first)
    }
  })

  it('keeps normal numeric order unchanged', () => {
    const rows = [[3], [1], [2]] as const
    expect(computeSortedRowOrder(rows.map((r) => [...r]), 0, true)).toEqual([1, 2, 0])
    expect(computeSortedRowOrder(rows.map((r) => [...r]), 0, false)).toEqual([0, 2, 1])
  })
})
